...
async function handleBikeState(chatId, bike) {
    ...
    if (bike === 'Да') {
        ...
        await bot.sendMessage(chatId, 'Иногда есть потребность в пеших курьерах. Если тебе это интересно, продолжим. ✅', keyboards.continue);
    }
}

async function handleWeightState(chatId, weightResponse) {
    userData[chatId].canCarryWeight = weightResponse;
...
async function handlePhoneState(chatId, phone) {
    ...
    if (!phoneRegex.test(phone)) {
        await bot.sendMessage(chatId, 'Пожалуйста, введите корректный номер телефона.', keyboards.remove);
        return;
    }
    ...
    const summary =
        ...
        Вес заказов: ${userData[chatId].canCarryWeight || 'Не указано'}\n
