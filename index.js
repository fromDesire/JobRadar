...
async function handleWalkCourierState(chatId, walkResponse) {
    if (walkResponse === 'Продолжим') {
        userStates[chatId] = 'WAITING_WEIGHT';
        await bot.sendMessage(chatId, await bot.sendMessage(chatId, '*Иногда заказ, который везёт курьер, может весить 15\\-20 кг.* Справишься? 🏋️', { parse_mode: 'MarkdownV2', ...keyboards.weight });
    } else {
